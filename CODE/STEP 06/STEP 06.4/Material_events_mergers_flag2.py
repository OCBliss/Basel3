...
process_directory = os.path.join(MAT_DIR, "Failures/Call Reports")
mergers_directory = os.path.join(MAT_DIR, "Mergers/Cleaned")
financial_quarter_file = os.path.join(MAT_DIR, "quarters_failures.csv")
output_directory = os.path.join(MAT_DIR, "Mergers/Call Reports")

# # process_directory = '/Users/johnnywaller/Desktop/Basel3/Call Report/CSV/Dynamic Lag/Cleaned'
# process_directory = '/Users/johnnywaller/Desktop/Basel3/Material Events/Failures/Call Reports'
# mergers_directory = '/Users/johnnywaller/Desktop/Basel3/Material Events/Mergers/Cleaned'
# financial_quarter_file = '/Users/johnnywaller/Desktop/Basel3/Material Events/quarters_failures.csv'
# output_directory = '/Users/johnnywaller/Desktop/Basel3/Material Events/Mergers/Call Reports'
...
mergers_data_list = []
for file in mergers_files:
    try:
        df = pd.read_csv(file)
        df['source_file'] = os.path.basename(file)  # tag the file
        mergers_data_list.append(df)
    except Exception as e:
        print(f"Error reading {file}: {e}")

mergers_data = pd.concat(mergers_data_list, ignore_index=True)

# Ensure CERT is numeric, coerce errors (non-numeric values become NaN)
mergers_data['CERT'] = pd.to_numeric(mergers_data['CERT'], errors='coerce')

# Identify bad rows
bad_rows = mergers_data[~mergers_data['CERT'].apply(np.isfinite)]
if not bad_rows.empty:
    print("\n⚠️ Found non-finite CERT values in these rows:")
    print(bad_rows[['CERT', 'source_file']])
    print("\nThese rows will be dropped to avoid crash.\n")

# Drop rows with bad CERT
mergers_data = mergers_data[mergers_data['CERT'].apply(np.isfinite)]

# Now safely convert
mergers_data['CERT'] = mergers_data['CERT'].astype(int).astype(str)
