...
OUTPUT_DIR = os.path.join(ROOT_DIR, "Material Events", "Failures", "RAW")
...
if __name__ == "__main__":
    df = fetch_fdic_history("2014-01-01", "2024-12-31", change_codes=[211])
    if df is not None:
        output_path = os.path.join(OUTPUT_DIR, "BankFind Suite API - Failures.csv")
        df.to_csv(output_path, index=False)
        print("✅ CSV saved as 'BankFind Suite API - failures.csv'")
