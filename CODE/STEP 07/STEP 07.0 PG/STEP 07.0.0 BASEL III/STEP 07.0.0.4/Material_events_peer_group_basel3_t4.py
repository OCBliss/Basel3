...
output_failures = os.path.join(MAT_DIR, "Failures/Peer Group Failures/Basel III T+4")
...
if not csv_files:
    ...
    for file in csv_files:
        ...
        # Keep only the rows with a '1' value in the header FAILURE_T4.
        data = data[data['FAILURE_T4'] == 1]
...
output_mergers = os.path.join(MAT_DIR, "Mergers/Peer Group Mergers/Basel III T+4")
...
if not csv_files2:
    ...
    for file in csv_files2:
        ...
        # Keep only the rows with a '1' value in the header MERGER_T1.
        data = data[data['MERGER_T4'] == 1]
...
output_survivors = os.path.join(MAT_DIR, "Survivors/Peer Group Survivors/Basel III T+4")
...
if not csv_files3:
    ...
    for file in csv_files3:
       ...
       # Convert FAILURE_T4 and MERGER_T4 to numeric in case they are strings.
        data['FAILURE_T4'] = pd.to_numeric(data['FAILURE_T4'], errors='coerce')
        data['MERGER_T4'] = pd.to_numeric(data['MERGER_T4'], errors='coerce')
        
        # Define a drop condition: any row where FAILURE_T4 == 1 OR MERGER_T1 == 1 should be removed.
        drop_condition = (data['FAILURE_T4'] == 1) | (data['MERGER_T4'] == 1)
