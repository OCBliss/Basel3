    ...
    if file_name.endswith('.csv'):
        try:
            ...
            for _, row in signals_df_rcb.iterrows():
                ...
                # ###---ZRWA
                # RCRI_ZRWA_Basel_entry1 = row['RCOND987'] if row['RCON9999'] >= 20150331 and row['RCOND987'] != 0 else row['RCONB696'] if row['RCON9999'] < 20150331 and row['RCONB696'] != 0 else 0
                # ###---ZRWA adjusted
                # ZRWA_Basel_entry1 = RCRI_ZRWA_Basel_entry1  - (((row['RCOND962']+row['RCOND967'])/(row['RCON1754']+row['RCON1773']))*(0.15*row['RCONA554'] + 0.1*row['RCONA553'] + 0.08*row['RCONA552'] + 0.15*row['RCONA560'] + 0.1*row['RCONA559'] + 0.08*row['RCONA558'])) if row['RCON9999'] >= 20150331 else 1
                ...
                entry1 = {
                    ...
                    'RCRI-P859-A223': row['RCOAP840'] / row['RCOAA223'] if row['RCOAA223'] != 0 else 0,
                    'RCRI-CET1-RWA+': row['RCOAP840'] / RWA_Basel_entry1 if RWA_Basel_entry1 != 0 else 0,
                ...
                # ###---ZRWA
                # RCRI_ZRWA_Basel_entry2 = row['RCOND987'] if row['RCON9999'] >= 20150331 else row['RCONB696']
                # ###---ZRWA adjusted
                # ZRWA_Basel_entry2 = RCRI_ZRWA_Basel_entry2 - (((row['RCOND962']+row['RCOND967'])/(row['RCON1754']+row['RCON1773']))*(0.15*row['RCFDA554'] + 0.1*row['RCFDA553'] + 0.08*row['RCFDA552'] + 0.15*row['RCFDA560'] + 0.1*row['RCFDA559'] + 0.08*row['RCFDA558']))
                ...
                entry2 = {
                    ...
                    'RCRI-P859-A223': row['RCFAP840'] / row['RCFAA223'] if row['RCFAA223'] != 0 else 0,
                    'RCRI-CET1-RWA+': row['RCFAP840'] / RWA_Basel_entry2 if RWA_Basel_entry2 != 0 else 0,
