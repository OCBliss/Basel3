...
def run_with_hash_check(step, input_dirs):
    ...
    script_args = []
    if "args" in SCRIPTS[step]:
        script_args = [str(x) for x in SCRIPTS[step]["args"]]

    stdout_lines = []
    try:
        # inject PIPE_INPUT_DIR from single parent if available (for hash probe run)
        env = os.environ.copy()
        parents = [dep for dep, dependents in DEPENDENCIES.items() if step in dependents]
        if len(parents) == 1 and parents[0] in STEP_OUTPUTS:
            env["PIPE_INPUT_DIR"] = STEP_OUTPUTS[parents[0]]

        process = subprocess.Popen(
            ["python", "-u", script_abs] + script_args,
...
def execute_step(step):
    ...
    try:
        # inject PIPE_INPUT_DIR from single parent if available
        env = os.environ.copy()
        parents = [dep for dep, dependents in DEPENDENCIES.items() if step in dependents]
        if len(parents) == 1 and parents[0] in STEP_OUTPUTS:
            env["PIPE_INPUT_DIR"] = STEP_OUTPUTS[parents[0]]
